...
def average(numbers):
    ...
    for num in numbers:
        the_sum = the_sum + num
    result = float(the_sum) / len(numbers)
    return result
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    For example::
        ...
        ['I', 'love', 'cookies']
    """
    # Not sure why my solution below doest work:

    # i = 0
    # reverse = items[::-1]
    # while i < len(items):
    #     items.pop(0)
    #     items.append(reverse[i])
    #     i = i + 1
    # return items
...
def duplicates(items):
    ...
    while i < len(items):
        ...
        if current_item in result_lst:
            i = i + 1
        else:
            j = i + 1
            while j < len(items):
                if current_item == items[j]:
                    result_lst.append(current_item)
                    break
                j = j + 1
            i = i + 1
